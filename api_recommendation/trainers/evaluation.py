...
def precision_at_k(k, ground_truth, predictions):
    ...
    if len(predictions) == 0:
        ...
        count = 0
        while i < k:
            if i < len(predictions) and predictions[i].endpoint in ground_truth:
                count += 1
            result[i] = count / (i + 1)
            i += 1
    return result
...
def f1_score_at_k(precision, recall):
    ...
    if all(precision_k == 0 for precision_k in precision) and all(recall_k == 0 for recall_k in recall):
        return result
    for i, (precision_k, recall_k) in enumerate(zip(precision, recall)):
    ...
    if len(predictions) == 0:
        ...
        while i < k:
            ...
            if i < len(predictions):
                if predictions[i].endpoint in ground_truth:
                    relevance = 1
